...
async function readHowAgentsPayMd(): Promise<string> {
  const candidates = [
    join(process.cwd(), 'docs', 'pay-per-execution', '25_HOW_AGENTS_PAY.md'),
    join(process.cwd(), '..', 'docs', 'pay-per-execution', '25_HOW_AGENTS_PAY.md'),
    join(process.cwd(), '..', '..', 'docs', 'pay-per-execution', '25_HOW_AGENTS_PAY.md')
  ];
  ...
  throw new Error('25_HOW_AGENTS_PAY.md not found');
}
...
export async function GET(req: Request) {
  try {
    const md = await readHowAgentsPayMd();

    if (wantsHtml(req)) {
      const html = renderMarkdownLiteToHtml(md, { title: 'How agents pay' });
