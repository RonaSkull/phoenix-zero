...
async function readAgentIntegrationContractMd(): Promise<string> {
  const candidates = [
    join(process.cwd(), 'docs', 'pay-per-execution', '20_AGENT_INTEGRATION_CONTRACT.md'),
    join(process.cwd(), '..', 'docs', 'pay-per-execution', '20_AGENT_INTEGRATION_CONTRACT.md'),
    join(process.cwd(), '..', '..', 'docs', 'pay-per-execution', '20_AGENT_INTEGRATION_CONTRACT.md')
  ];
  ...
  throw new Error('20_AGENT_INTEGRATION_CONTRACT.md not found');
}
...
export async function GET(req: Request) {
  try {
    const md = await readAgentIntegrationContractMd();

    if (wantsHtml(req)) {
      const html = renderMarkdownLiteToHtml(md, { title: 'Agent Integration Contract' });
