(function () {
  ...
  async function runOne(hostEl) {
    if (!hostEl || hostEl.__phoenixZeroLiveMounted) return;
    hostEl.__phoenixZeroLiveMounted = true;

    var apiBase = (hostEl.getAttribute('data-api-base') || '').trim();
    var jobId = (hostEl.getAttribute('data-job-id') || hostEl.getAttribute('data-live-job-id') || '').trim();
    ...
    var endpoint = apiBase.replace(/\/+$/g, '') + '/api/global-live-auth?jobId=' + encodeURIComponent(jobId);
    ...
    try {
      ...
      if (verified === true) {
        ...
        setBadgeState(badge, 'not_verified');
      } else if (decision === 'verified' || decision === 'verified_unregistered_creator') {
        setBadgeState(badge, 'verified');
      } else {
        setBadgeState(badge, 'not_verified');
      }
  ...
  function runAll() {
    var nodes = document.querySelectorAll('[data-phoenix-zero-live-embed]');
  ...
  try {
    window.PhoenixZeroLiveEmbed = { runAll: runAll, runOne: runOne };
  } catch {
  }
