(function () {
  ...
  async function runOne(hostEl) {
    if (!hostEl || hostEl.__phoenixZeroMounted) return;
    hostEl.__phoenixZeroMounted = true;

    var apiBase = (hostEl.getAttribute('data-api-base') || '').trim();
    var videoUrl = (hostEl.getAttribute('data-video-url') || '').trim();
    ...
    var endpoint = apiBase.replace(/\/+$/g, '') +
      '/api/global-auth?videoUrl=' + encodeURIComponent(videoUrl) +
    ...
    try {
      ...
      if (decision === 'verified' || decision === 'verified_unregistered_creator') {
  ...
  function runAll() {
    var nodes = document.querySelectorAll('[data-phoenix-zero-embed]');
  ...
  try {
    window.PhoenixZeroEmbed = { runAll: runAll, runOne: runOne };
  } catch {
  }
