...
async function main() {
  ...
  const imageBytes = new Uint8Array(await readFile(inPath));
  ...
  if (proof.fingerprint?.alg === 'dhash_v1') {
    const extracted = await computeImageDHashB64Url({ imageBytes, width: proof.fingerprint.width, height: proof.fingerprint.height });
